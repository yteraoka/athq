...
func (m tuiModel) helpLine() string {
	...
	case m.focus == paneCatalog:
		hints = "tab pane · enter expand · i/click insert · r reload · ^r run · ^s save · ^w/^o saved query · ^l $EDITOR · q quit"
	case m.focus == paneColumns:
		hints = "tab pane · i/click insert · ←back · ^r run · ^s save · ^w/^o saved query · ^l $EDITOR · q quit"
	...
	case m.focus == paneEditor && m.vim.on && m.vim.mode == vimNormal:
		hints = "i insert · v visual · yy/dd/p copy/cut/paste · u undo · tab pane · esc leave editor · ^r run · ^s save · ^w/^o saved query · ^l $EDITOR"
	case m.focus == paneEditor:
		...
		hints = "tab complete · esc " + m.escapeHint() + " · ^y copy · ^v paste · ^r run · ^s save · ^w/^o saved query · ^l $EDITOR · ^c cancel/quit"
	default:
		hints = "↑↓←→ scroll · tab pane · ^s save · ^w/^o saved query · ^l $EDITOR · q quit"
	}

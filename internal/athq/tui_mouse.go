...
func (m tuiModel) hitTest(y int) (paneHit, bool) {
	...
	// The header takes the first line, then the two panes, the editor and the
	// result follow one another.
	topTop := 1
	editTop := topTop + m.topHeight
	resTop := editTop + m.editHeight
